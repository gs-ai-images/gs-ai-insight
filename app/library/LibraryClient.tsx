...
export default function LibraryClient({ initialDbPosts = [] }: { initialDbPosts?: LibraryPost[] }) {
  const { data: session } = useSession();
  const isAdmin = session?.user?.role === 'ADMIN';

  const [posts, setPosts] = useState<LibraryPost[]>(initialDbPosts);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Only update if length changed or we really need to 
          // (for simple SSR hydration, this background refresh covers external DB changes)
          if (JSON.stringify(mappedPosts) !== JSON.stringify(posts)) {
    ...
    // Only refresh if we didn't get initial server posts, or occasionally
    if (initialDbPosts.length === 0) {
       fetchPosts();
    }
  }, [session, initialDbPosts.length, posts]);
    ...
    <div className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8 w-full pointer-events-auto">
      ...
      {loading && filteredPosts.length === 0 ? (
        <div className="py-20 text-center w-full">
           ...
           <h3 className="text-xl font-bold text-gray-300">데이터를 불러오는 중입니다...</h3>
        </div>
