...
async function processSeeds() {
  console.log('Starting background seed process for 18 URLs...');
  ...
  for (const item of urls) {
    ...
    // Rate limit delay slightly to prevent Anthropic hitting limits immediately
    await new Promise(r => setTimeout(r, 2000));
  }
  console.log('Seed Background Process Done!');
}

export async function GET() {
  // Fire and forget
  processSeeds().catch(console.error);
  return NextResponse.json({ message: 'Seed background process started.' });
}
