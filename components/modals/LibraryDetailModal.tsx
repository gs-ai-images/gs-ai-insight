...
interface LibraryDetailModalProps {
  ...
  onEdit?: (post: LibraryPost) => void;
}

export default function LibraryDetailModal({ isOpen, onClose, post, onDelete, onEdit }: LibraryDetailModalProps) {
  ...
  const handleDelete = () => {
    if (confirm("정말 이 게시물을 삭제하시겠습니까?")) {
      ...
      onClose();
      setTimeout(() => alert("게시물이 삭제되었습니다."), 50);
    }
  };
        ...
        <div className="px-6 pb-12 md:px-12 max-w-4xl w-full mx-auto flex-grow -mt-4">
                ...
                <div className="flex gap-2">
                  {onEdit && (
                    <button 
                      ...
                      className="text-sm text-white bg-amber-600 hover:bg-amber-700 transition px-4 py-2 rounded-lg font-bold shadow-sm cursor-pointer flex items-center gap-1.5"
                    >
                      <i className="fa-solid fa-pen-to-square"></i> 게시물 수정
                    </button>
                  )}
                  <button 
          ...
          {imagesToRender.length > 0 && (
            <div className="mb-8 space-y-4">
              {imagesToRender.map((mediaUrl, idx) => (
                <div key={idx} className="w-full flex justify-center bg-gray-50 rounded-2xl border border-gray-200 shadow-inner overflow-hidden">
