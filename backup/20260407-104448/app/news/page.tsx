...
export default function NewsPage() {
  ...
  const fetchPosts = useCallback(async () => {
    try {
      ...
      // If no news posts, seed the initial ones
      if (mapped.length === 0) {
  ...
  const handleAddOrEditPost = async (submittedPost: NewsPost) => {
    ...
    if (isNew) {
      ...
      await fetch(`/api/posts/${submittedPost.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
...
{loading && <div className="mt-4 text-rose-500 font-bold"><i className="fa-solid fa-spinner fa-spin mr-2"></i> 뉴스 불러오는 중...</div>}
        </div>
      ) : (
